...
namespace DakLakCoffeeSupplyChain.Services.Services
{
    public class ContractDeliveryItemService : IContractDeliveryItemService
    {
        ...
        public async Task<IServiceResult> GetByDeliveryBatchId(Guid deliveryBatchId, Guid userId)
        {
            try
            {
                ...
                // Lấy danh sách ContractDeliveryItems theo DeliveryBatchId
                var contractDeliveryItems = await _unitOfWork.ContractDeliveryItemRepository.GetAllAsync(
                    predicate: cdi =>
                       ...
                       !cdi.IsDeleted &&
                       cdi.DeliveryBatch.Contract.SellerId == managerId,
                    include: query => query
                       .Include(cdi => cdi.ContractItem)
                          .ThenInclude(ci => ci.CoffeeType)
                       .Include(cdi => cdi.DeliveryBatch)
                          .ThenInclude(db => db.Contract),
                    orderBy: query => query.OrderBy(cdi => cdi.DeliveryItemCode),
                    asNoTracking: true
                );

                // Kiểm tra nếu không có dữ liệu
                if (contractDeliveryItems == null || !contractDeliveryItems.Any())
                {
                    return new ServiceResult(
                        Const.WARNING_NO_DATA_CODE,
                        Const.WARNING_NO_DATA_MSG,
                        new List<ContractDeliveryItemViewDto>()
                    );
                }

                // Map sang DTO
                var contractDeliveryItemDtos = contractDeliveryItems
                    .Select(item => item.MapToContractDeliveryItemViewDto())
                    .ToList();

                return new ServiceResult(
                    Const.SUCCESS_READ_CODE,
                    Const.SUCCESS_READ_MSG,
                    contractDeliveryItemDtos
                );
            }
            catch (Exception ex)
            {
        ...
        public async Task<IServiceResult> SoftDeleteContractDeliveryItemById(Guid deliveryItemId, Guid userId)
        {
            try
            {
