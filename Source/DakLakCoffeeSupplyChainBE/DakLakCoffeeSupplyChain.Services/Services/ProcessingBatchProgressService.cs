...

        {
...

            {
...

                {
                    ...
                    // 🔧 VALIDATION: Kiểm tra khối lượng ra với batch input
    ...
    bool isManager)
...

        {
...

            {
                ...
                // 🔧 FIX: Lấy evaluation FAIL cuối cùng (không phải evaluation mới nhất)
                ...
                // 🔧 FIX: Tìm evaluation FAIL cuối cùng (không phải evaluation retry với result null/empty)
                ...
                // 🔧 FIX: Lấy StageId từ failed stages thay vì từ input (vì frontend không gửi StageId)
                ...
                // 🔧 MỚI: Cho phép frontend chỉ định StageId cụ thể để retry
                ...
                // Kiểm tra xem frontend có gửi StageId không
                if (input.StageId.HasValue)
                {
                    Console.WriteLine($"DEBUG UPDATE AFTER EVALUATION: Frontend selected StageId: {input.StageId.Value}");
                    currentStage = failedStages.FirstOrDefault(s => s.StageId == input.StageId.Value);
                    
                if (currentStage == null)

                {

                        Console.WriteLine($"DEBUG UPDATE AFTER EVALUATION: Selected StageId {input.StageId.Value} not found in failed stages!");
                        return CreateValidationError("SelectedStageIdNotFoundInFailedStages", new Dictionary<string, object>
                        {
                            ["SelectedStageId"] = input.StageId.Value,
                            ["AvailableFailedStageIds"] = string.Join(", ", failedStages.Select(s => s.StageId))
                        });
                    }
                }
                
                // Fallback: Nếu không có StageId được chỉ định, lấy giai đoạn đầu tiên
                if (currentStage == null)
                {
                    Console.WriteLine($"DEBUG UPDATE AFTER EVALUATION: No specific StageId selected, using first failed stage");
                    currentStage = failedStages.FirstOrDefault();
                }
                
                if (currentStage == null)
                {
                    Console.WriteLine($"DEBUG UPDATE AFTER EVALUATION: ERROR - No failed stages found!");
                    return CreateValidationError("NoFailedStagesToUpdate", new Dictionary<string, object>
                    {
                        ["BatchId"] = batchId.ToString()
                    });
                ...
                // 🔧 FIX: Kiểm tra xem stage có tồn tại trong stages list không
                ...
                Console.WriteLine($"DEBUG UPDATE AFTER EVALUATION: Using stage: {currentStage.StageName} (ID: {currentStage.StageId}) from failed stages list");
                ...
                // 🔧 CẢI THIỆN: Smart Validation cho khối lượng dựa trên loại stage
...

                {
...

                    {
                        ...
                        // 🔧 VALIDATION CHO STAGE BỊ FAIL: Áp dụng validation nghiêm ngặt với logic waste
                        ...
                        var validationResult = await ValidateRetryQuantityWithWasteAsync(
                            ...
                            latestProgress.OutputQuantity.Value, 

                            latestProgress.OutputUnit,
...

                    {
                        ...
                        // 🔧 VALIDATION CHO STAGE KHÔNG BỊ FAIL: Chỉ kiểm tra cơ bản
                ...
                // 🔧 MỚI: Tính waste cho progress retry

                var finalOutputBeforeRetry = await GetFinalOutputBeforeRetryAsync(batchId);

                var retryQuantityInKg = await NormalizeQuantityAsync(input.OutputQuantity.Value, input.OutputUnit ?? "kg");

                var wasteQuantity = finalOutputBeforeRetry - retryQuantityInKg;

                
                
                Console.WriteLine($"DEBUG WASTE CALCULATION: Final output before retry: {finalOutputBeforeRetry} kg");

                Console.WriteLine($"DEBUG WASTE CALCULATION: Retry quantity: {retryQuantityInKg} kg");

                Console.WriteLine($"DEBUG WASTE CALCULATION: Calculated waste: {wasteQuantity} kg");
                ...
                // 🔧 MỚI: Tạo waste record nếu có waste
...

                {
...

                    {
                        ...
                        WasteType = "Phế phẩm retry", // Loại waste cho retry

                        Quantity = wasteQuantity,

                        Unit = "kg",

                        Note = $"Waste từ retry stage {currentStage.StageName} - Output: {retryQuantityInKg} kg từ {finalOutputBeforeRetry} kg",
                ...
                // 🔧 MỚI: Kiểm tra xem tất cả failed stages đã được retry chưa
                ...
                // 🔧 MỚI: Tính toán stages còn lại cần retry
        ...
        // ================== RETRY QUANTITY VALIDATION ==================

        
        
        /// <summary>

        /// Validation result cho retry quantity
        ...
        /// Smart validation cho khối lượng retry dựa trên stage và context
...

        {
            ...
            // 🔧 MỚI: Dynamic tolerance dựa trên stage type
            ...
            // 🔧 MỚI: Comprehensive validation rules
        ...
        /// Lấy tolerance dựa trên stage type
...

        {
...

            {
                ...
                "thu hoạch" => 0.15, // 15% - ít biến động

                "phơi" => 0.30,      // 30% - có thể thay đổi nhiều do thời tiết

                "xay vỏ" => 0.20,    // 20% - trung bình

                "rang" => 0.25,       // 25% - có thể thay đổi do nhiệt độ

                "đóng gói" => 0.10,   // 10% - ít biến động
        ...
        /// Lấy warning threshold dựa trên stage type
...

        {
...

            {
                ...
                "thu hoạch" => 0.10, // 10% warning

                "phơi" => 0.20,      // 20% warning

                "xay vỏ" => 0.15,    // 15% warning

                "rang" => 0.20,      // 20% warning

                "đóng gói" => 0.05,  // 5% warning
        ...
        /// Lấy validation rules dựa trên stage type
...

        {
            ...
            // Rule 1: Không được tăng quá tolerance
            ...
            // Rule 2: Không được giảm quá nhiều
...

            {
                ...
                Threshold = 0.20, // 20% max decrease
            ...
            // Rule 3: Không được bằng 0 hoặc âm
            ...
            // Rule 4: Không được quá lớn (tránh nhập sai)
        ...
        /// Chuyển đổi khối lượng về kg để so sánh
...

        {
...

            {
                ...
                "lang" => quantity * 0.0375, // 1 lạng = 37.5g

                "lb" => quantity * 0.453592, // 1 pound = 0.453592 kg

                "oz" => quantity * 0.0283495, // 1 ounce = 0.0283495 kg

                _ => quantity // Default to kg
        ...
        /// Validation cho stage không bị fail (validation nhẹ nhàng hơn)
...

        {
            ...
            // 🔧 VALIDATION NHẸ NHÀNG: Chỉ kiểm tra cơ bản cho stage không bị fail

            var relaxedTolerance = GetToleranceForStage(currentStage.StageName) * 2; // Tăng gấp đôi tolerance
            ...
            // 2. Không được quá lớn (tránh nhập sai)
            ...
            // 3. Không được tăng quá nhiều (tolerance gấp đôi)
            ...
            // 4. Không được giảm quá nhiều (tolerance gấp đôi)
        ...
        /// Lấy suggestion dựa trên stage và error
        ...
        /// Kiểm tra khối lượng không vượt quá crop progress cuối cùng
...

        {
...

            {
...

                {
                    ...
                    return result; // Không có crop season thì bỏ qua validation

                }



                // Lấy crop season detail để lấy ActualYield
...

                {
                    ...
                    return result; // Không có actual yield thì bỏ qua validation
                ...
                var normalizedCropQuantity = await NormalizeQuantityAsync(
                    ...
                    "kg" // Giả sử ActualYield được lưu bằng kg
                ...
                // Kiểm tra nếu output vượt quá actual yield
...

            {
                ...
                return result; // Nếu có lỗi thì bỏ qua validation
        ...
        /// Validation cho retry quantity với logic waste
...

        {
            ...
            // 1. Lấy output cuối cùng trước retry

            var finalOutputBeforeRetry = await GetFinalOutputBeforeRetryAsync(batchId);

            Console.WriteLine($"DEBUG WASTE VALIDATION: Final output before retry: {finalOutputBeforeRetry} kg");
            ...
            var finalOutputInKg = finalOutputBeforeRetry;



            // 3. 🔧 VALIDATION: Không được vượt quá output cuối cùng trước retry
...

            {
                ...
                result.ErrorCode = "RetryQuantityExceedsFinalOutput";
...

                {
                    ...
                    ["MaxAllowed"] = finalOutputInKg,

                    ["MaxAllowedUnit"] = "kg",

                    ["StageName"] = currentStage.StageName,

                    ["Note"] = "Không thể vượt quá khối lượng cuối cùng trước retry"
            ...
            // 4. 🔧 VALIDATION: Tính waste và kiểm tra tính hợp lý

            var waste = finalOutputInKg - retryQuantityInKg;

            Console.WriteLine($"DEBUG WASTE VALIDATION: Calculated waste: {waste} kg");
...

            {
                ...
                result.ErrorCode = "InvalidWasteCalculation";

                result.ErrorParameters = new Dictionary<string, object>

                {

                    ["FinalOutputBeforeRetry"] = finalOutputInKg,

                    ["RetryQuantity"] = retryQuantityInKg,

                    ["CalculatedWaste"] = waste,

                    ["StageName"] = currentStage.StageName,

                    ["Note"] = "Waste không được âm"
            ...
            // 5. 🔧 VALIDATION: Kiểm tra tỷ lệ waste có hợp lý không

            var wastePercentage = (waste / finalOutputInKg) * 100;
...

            {
...

                {
                    ...
                    ["Note"] = $"Tỷ lệ waste quá cao cho stage {currentStage.StageName}"
        ...
        /// Lấy output cuối cùng trước retry
...

        {
            ...
            // Lấy progress cuối cùng không bị fail
        ...
        /// Lấy tỷ lệ waste tối đa cho từng stage
...

        {
...

            {
                ...
                "thu hoạch" => 20.0, // 20% waste tối đa cho thu hoạch

                "phơi" => 15.0,      // 15% waste tối đa cho phơi

                "xay vỏ" => 25.0,    // 25% waste tối đa cho xay vỏ

                "phân loại" => 10.0, // 10% waste tối đa cho phân loại

                "rang" => 20.0,      // 20% waste tối đa cho rang

                "đóng gói" => 5.0,   // 5% waste tối đa cho đóng gói

                _ => 30.0            // 30% waste tối đa cho các stage khác
        ...
        /// <summary>
        /// API mới: Cập nhật progress cho các giai đoạn tiếp theo (không bị fail)
        /// </summary>
            ...
            bool isManager)
        {
            try
            {
                ...
                // 🔧 KIỂM TRA: Cho phép cập nhật khi batch đang ở InProgress hoặc AwaitingEvaluation
                ...
                // 🔧 VALIDATION: Kiểm tra khối lượng output
                if (!input.OutputQuantity.HasValue || input.OutputQuantity.Value <= 0)
                {
                    return CreateValidationError("OutputQuantityMustBePositive", new Dictionary<string, object>
                    {
                ...
                // Tạo progress mới cho stage tiếp theo
                var progress = new ProcessingBatchProgress
                {
                    ProgressId = Guid.NewGuid(),
                    BatchId = batchId,
                ...
                // 🔧 QUAN TRỌNG: Lưu tất cả thay đổi
